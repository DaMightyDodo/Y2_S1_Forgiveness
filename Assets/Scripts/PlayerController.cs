...
public class PlayerController : MonoBehaviour
{
    ...
    [SerializeField] private Stats stats; //Scriptable Object
...
{
    platformCheck.SetDropping(isDropping);
}
    #endregion
    ...
    private void HandleMovement()
    {
        ...
        {
            float targetSpeed = _horizontal * stats.maxSpeed;
            // Apply apex multiplier while in air after reaching jump apex
            if (!_isGrounded && _reachedApex)
            {
                targetSpeed *= stats.apexMultiplier;
            }
            if (_isGrounded && _isCrouching)
            {
                targetSpeed *= stats.crouchMultiplier;
            }

            _frameVelocity.x = Mathf.MoveTowards(_frameVelocity.x, targetSpeed, stats.acceleration * Time.fixedDeltaTime);

            // Prevent walking off ledge while crouching and grounded
            if (_isCrouching && _isGrounded && Mathf.Abs(_frameVelocity.x) > 0.01f)
            {
                Bounds b = _col.bounds;
                LayerMask mask = ~stats.playerLayer;

                // 4 downward rays
                Vector2 leftOuter  = new Vector2(b.min.x, b.min.y);
                Vector2 leftInner  = new Vector2(b.center.x - b.extents.x * stats.innerRayOffset - stats.rayOffsetX, b.min.y);
                Vector2 rightInner = new Vector2(b.center.x + b.extents.x * stats.innerRayOffset + stats.rayOffsetX, b.min.y);
                Vector2 rightOuter = new Vector2(b.max.x, b.min.y);

                bool leftOuterHit  = Physics2D.Raycast(leftOuter, Vector2.down, stats.ledgeCheckDistance, mask);
                bool leftInnerHit  = Physics2D.Raycast(leftInner, Vector2.down, stats.ledgeCheckDistance, mask);
                bool rightInnerHit = Physics2D.Raycast(rightInner, Vector2.down, stats.ledgeCheckDistance, mask);
                bool rightOuterHit = Physics2D.Raycast(rightOuter, Vector2.down, stats.ledgeCheckDistance, mask);

                Debug.DrawRay(leftOuter, Vector2.down * stats.ledgeCheckDistance, leftOuterHit ? Color.red : Color.green);
                Debug.DrawRay(leftInner, Vector2.down * stats.ledgeCheckDistance, leftInnerHit ? Color.red : Color.green);
                Debug.DrawRay(rightInner, Vector2.down * stats.ledgeCheckDistance, rightInnerHit ? Color.red : Color.green);
                Debug.DrawRay(rightOuter, Vector2.down * stats.ledgeCheckDistance, rightOuterHit ? Color.red : Color.green);
    ...
    {
        bool canUseCoyote = _coyoteAble && !_isGrounded && _time < _frameLeftGrounded + stats.coyoteTime && _time < 
            _timeJumpWasPressed + stats.bufferTime;
        bool canUseBuffer = _isGrounded && _time < _timeJumpWasPressed + stats.bufferTime;
        ...
        if (canUseCoyote || canUseBuffer)
        {
            ...
            _rb.linearVelocity = new Vector2(_rb.linearVelocity.x, stats.jumpForce); //jump!
            _coyoteAble = false;
...
private void CheckCollisions()
{
    ...
    RaycastHit2D hit = Physics2D.BoxCast(_col.bounds.center, _col.size, 0, Vector2.down, stats.grounderDistance, ~stats.playerLayer);
    ...
    private void BumpHeadCorrection()
    {
        ...
        Bounds b = _col.bounds;
        float rayLength = stats.ceilingCheckDistance;
        LayerMask mask = ~stats.playerLayer;

        // 4 rays at the top of the _collider
        Vector2 leftOuter = new Vector2(b.min.x - stats.outerRayOffset, b.max.y);
        Vector2 leftInner = new Vector2(b.center.x - b.extents.x * stats.innerRayOffset, b.max.y);
        Vector2 rightInner = new Vector2(b.center.x + b.extents.x * stats.innerRayOffset, b.max.y);
        Vector2 rightOuter = new Vector2(b.max.x + stats.outerRayOffset, b.max.y);
        ...
        {
            _rb.position += Vector2.left * stats.cornerCorrectionDistance; // slide left around ledge
        }
        else if (leftOuterHit && !leftInnerHit && !rightInnerHit && !rightOuterHit)
        {
            _rb.position += Vector2.right * stats.cornerCorrectionDistance; // slide right around ledge
        }
    ...
    private void CatchJumpCorrection()
    {
        ...
        float rayLength = stats.edgeRayLength;
        ...
        {
            // facing right
            bottomOrigin = new Vector2(b.max.x, b.min.y + stats.bottomRayOffset);
            topOrigin = new Vector2(b.max.x, b.min.y + stats.edgeInnerHeight + stats.topRayOffset);
        }
        else
        {
            // facing left
            bottomOrigin = new Vector2(b.min.x, b.min.y + stats.bottomRayOffset);
            topOrigin = new Vector2(b.min.x, b.min.y + stats.edgeInnerHeight + stats.topRayOffset);
        }
        //1 RAYCAST ONLY BRUV
        ...
        if (bottomHit && !topHit)
        {
            _rb.position += Vector2.up * stats.edgeCorrectionDistance;
            _isGrounded = true;
            _coyoteAble = true;
            _reachedApex = false;
    ...
    private void HandleGravity()
    {
        ...
        {
            _rb.linearVelocityY = stats.groundingForce;
        ...
        else
        {
            ...
            {
                inAirGravity *= stats.apexLowGravityMultiplier;
