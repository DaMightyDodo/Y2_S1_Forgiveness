...
public class PlayerController : MonoBehaviour
{
    ...
    {
        var canUseCoyote = _coyoteAble && !_isGrounded && _time < _frameLeftGrounded + _stats.coyoteTime && _time <
            _timeJumpWasPressed + _stats.bufferTime;
        var canUseBuffer = _isGrounded && _time < _timeJumpWasPressed + _stats.bufferTime;
    ...
    private void CheckCollisions()
    {
        ...
        var hit = Physics2D.BoxCast(_col.bounds.center, _col.size, 0, Vector2.down, _stats.grounderDistance, ~_stats.playerLayer);
